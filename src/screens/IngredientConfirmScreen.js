      ...
      <ScrollView contentContainerStyle={styles.container} keyboardShouldPersistTaps="handled">
        ...
        <View style={styles.headerRow}>
          <View style={styles.headerCopy}>
            <Text style={styles.eyebrow}>{source === 'camera' ? 'AI 분석 결과 · 데모' : '직접 입력한 재료'}</Text>
        ...
        {uncertainCount > 0 ? (
          <View style={styles.warningBox}>
            <Text style={styles.warningText}>
              확인이 필요한 재료가 {uncertainCount}개 있어요. 맞지 않으면 삭제해 주세요.
            </Text>
          </View>
        ) : null}
...
const styles = StyleSheet.create({
  ...
  headerCopy: { flex: 1 },
  eyebrow: { color: colors.primary, fontSize: 12, fontWeight: '900', marginBottom: 7 },
  title: { color: colors.text, fontSize: 25, fontWeight: '900', lineHeight: 34 },
  countBadge: { backgroundColor: colors.primarySoft, borderRadius: 20, marginLeft: 12, paddingHorizontal: 12, paddingVertical: 8 },
  ...
  warningBox: { backgroundColor: '#FFF8E8', borderRadius: 12, marginTop: 18, padding: 13 },
  warningText: { color: colors.warning, fontSize: 12, fontWeight: '700', lineHeight: 18 },
  chipContainer: { flexDirection: 'row', flexWrap: 'wrap', marginBottom: 10, marginTop: 22 },
  label: { color: colors.text, fontSize: 14, fontWeight: '800', marginBottom: 9, marginTop: 14 },
  inputRow: { flexDirection: 'row' },
  input: { backgroundColor: colors.surface, borderColor: colors.border, borderRadius: 13, borderWidth: 1, color: colors.text, flex: 1, fontSize: 14, paddingHorizontal: 14, paddingVertical: 13 },
  addButton: { backgroundColor: colors.text, borderRadius: 13, justifyContent: 'center', marginLeft: 8, paddingHorizontal: 19 },
  addButtonText: { color: '#FFFFFF', fontSize: 14, fontWeight: '800' },
  excludeInput: { backgroundColor: colors.surface, borderColor: colors.border, borderRadius: 13, borderWidth: 1, color: colors.text, fontSize: 14, paddingHorizontal: 14, paddingVertical: 13 },
  helper: { color: colors.textMuted, fontSize: 11, marginLeft: 3, marginTop: 6 },
  searchButton: { alignItems: 'center', backgroundColor: colors.primary, borderRadius: 15, marginTop: 30, padding: 17 },
